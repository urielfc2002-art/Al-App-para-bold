...
export function FormulaGenerator({ onBack }: FormulaGeneratorProps) {
  ...
  // Estados para el flujo de trabajo de corte
  ...
  // Estados para crear nueva calculadora
  ...
  // Estados para ejecutar calculadora
  ...
  useEffect(() => {
    const saved = localStorage.getItem('customCalculators');
    if (saved) {
      try {
  ...
  const saveCalculators = (newCalculators: CustomCalculator[]) => {
    ...
    localStorage.setItem('customCalculators', JSON.stringify(newCalculators));
  };
  ...
  // Calcular automáticamente cuando cambien las medidas
  useEffect(() => {
  ...
  const handleAddToPackage = () => {
    ...
    const currentPieces = JSON.parse(localStorage.getItem('formulaGeneratorPackagePieces') || '[]');
    const nextWindowNumber = currentPieces.length > 0 ? 
      Math.max(...currentPieces.map((p: any) => {
        const parts = p.windowType.split(' ');
  ...
  // Vista de paquete de piezas de fórmulas
  if (showFormulaPackagePieces) {
    return (
      <FormulaPackagePieces
        onBack={() => setShowFormulaPackagePieces(false)}
      />
    );
  }

  // Vista principal - Lista de calculadoras
  if (currentView === 'list') {
    return (
  ...
  // Vista de creación y edición de calculadora
        ...
        <div className="w-full max-w-4xl bg-white rounded-lg p-4 sm:p-6 mt-8">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              placeholder="Ej: Ventana Europea, Puerta Francesa, etc."
            />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Jamba, Riel, Marco"
                />
              </div>
          ...
          {/* Botón para guardar calculadora */}
  ...
  // Vista de ejecución de calculadora
        ...
        <div className="w-full max-w-2xl bg-white rounded-lg p-6 mt-8">
          ...
          {results.length > 0 && (
            <div className="border-t pt-6">
              ...
              {/* Botones de acción después del título de resultados */}
