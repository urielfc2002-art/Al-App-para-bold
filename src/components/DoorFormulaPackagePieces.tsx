...
export function DoorFormulaPackagePieces({ onBack }: DoorFormulaPackagePiecesProps) {
  const [pieces, setPieces] = useState<ProfilePiece[]>(() =>
    JSON.parse(localStorage.getItem('doorFormulaGeneratorPackagePieces') || '[]')
  );
  ...
  const handleClearAll = () => {
    if (confirm('¿Estás seguro que deseas eliminar todas las piezas de fórmulas de puertas? Esta acción no se puede deshacer.')) {
      localStorage.setItem('doorFormulaGeneratorPackagePieces', '[]');
      setPieces([]);
    }
  };
  ...
  const handleSavePackage = (name: string) => {
    ...
    const savedPackages = JSON.parse(localStorage.getItem('savedDoorFormulaPiecePackages') || '[]');
    savedPackages.push(packageData);
    localStorage.setItem('savedDoorFormulaPiecePackages', JSON.stringify(savedPackages));

    setShowSaveModal(false);
    alert('¡Paquete de fórmulas de puertas guardado exitosamente!');
  };
      ...
      <div className="min-h-screen bg-[#003366] flex flex-col items-center px-4 animate-fade-in">
        ...
        <div className="text-center my-8">
          ...
          <div className="bg-white text-[#003366] px-6 py-2 rounded-full mt-4">
            <span className="font-bold">FÓRMULAS PERSONALIZADAS - PUERTAS</span>
          </div>
              ...
              <div className="bg-blue-50 rounded-lg py-3 px-4 mb-4">
                <div className="flex items-center justify-center gap-3">
                  <span className="font-mono text-blue-600 text-5xl font-bold">
                    #{currentPiece.doorType.split(' ').pop()}
                  </span>
                  <span className="text-[#003366] text-4xl font-bold">
                    {currentPiece.doorType.split(' ').slice(0, -1).join(' ')}
                  </span>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-[#003366] flex flex-col items-center px-4 animate-fade-in">
      ...
      <div className="text-center my-8">
        ...
        <div className="bg-white text-[#003366] px-6 py-2 rounded-full mt-4">
          <span className="font-bold">PIEZAS DE CALCULADORAS PERSONALIZADAS - PUERTAS</span>
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl bg-white rounded-lg p-6 mb-8">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-blue-600 text-lg font-bold">
                          #{piece.doorType.split(' ').pop()}
                        </span>
                        <span className="font-medium">
                          {piece.doorType.split(' ').slice(0, -1).join(' ')}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        {pieces.length === 0 && (
          <div className="text-center text-gray-500 py-8">
            ...
            <h3 className="text-xl font-bold mb-2">No hay piezas de fórmulas de puertas</h3>
      ...
      {showSaveModal && (
        <SaveDoorFormulaPiecePackageModal
      ...
      {showLoadModal && (
        <SavedDoorFormulaPiecePackagesModal
