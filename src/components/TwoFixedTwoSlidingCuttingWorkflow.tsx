...
interface TwoFixedTwoSlidingCuttingWorkflowProps {
  onBack: () => void;
  measurements: {
...
export function TwoFixedTwoSlidingCuttingWorkflow({ onBack, measurements, zocloSelection }: TwoFixedTwoSlidingCuttingWorkflowProps) {
  ...
  const steps: Step[] = [
    ...
    // 4. Ventila Fija - Cercochapa (2 pieces)
    ...Array(measurements.ventilaFijaCercochapa.pieces).fill(null).map((_, index) => ({
      title: "VENTILA FIJA - CERCOCHAPA",
      description: "Cortar cercochapa fijo",
      measure: measurements.ventilaFijaCercochapa.measure,
      currentPiece: index + 1,
      totalPieces: measurements.ventilaFijaCercochapa.pieces,
    })),

    // 5. Ventila Corrediza - Cercochapa (2 pieces)
    ...Array(measurements.ventilaCorrCercochapa.pieces).fill(null).map((_, index) => ({
      title: "VENTILA CORREDIZA - CERCOCHAPA",
      description: "Cortar cercochapa corredizo",
      measure: measurements.ventilaCorrCercochapa.measure,
      currentPiece: index + 1,
      totalPieces: measurements.ventilaCorrCercochapa.pieces,
    })),

    // 6. Ventila Fija - Traslape (2 pieces)
    ...Array(measurements.ventilaFijaTraslape.pieces).fill(null).map((_, index) => ({
      title: "VENTILA FIJA - TRASLAPE",
      description: "Cortar traslape fijo",
      measure: measurements.ventilaFijaTraslape.measure,
      currentPiece: index + 1,
      totalPieces: measurements.ventilaFijaTraslape.pieces,
    })),

    // 7. Ventila Corrediza - Traslape (2 pieces)
    ...Array(measurements.ventilaCorrTraslape.pieces).fill(null).map((_, index) => ({
      title: "VENTILA CORREDIZA - TRASLAPE",
      description: "Cortar traslape corredizo",
      measure: measurements.ventilaCorrTraslape.measure,
      currentPiece: index + 1,
      totalPieces: measurements.ventilaCorrTraslape.pieces,
    })),

    // 8. Zoclo Superior (4 pieces)
