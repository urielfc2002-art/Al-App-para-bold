...
interface SavedFormulaPiecePackage {
  id: string;
  name: string;
  date: string;
  pieces: any[];
...
interface SavedFormulaPiecePackagesModalProps {
  onClose: () => void;
  onLoadPackage: (packageData: SavedFormulaPiecePackage) => void;
}

export function SavedFormulaPiecePackagesModal({ onClose, onLoadPackage }: SavedFormulaPiecePackagesModalProps) {
  const [savedPackages, setSavedPackages] = useState<SavedFormulaPiecePackage[]>([]);
  const [selectedPackage, setSelectedPackage] = useState<SavedFormulaPiecePackage | null>(null);

  useEffect(() => {
    const packages = localStorage.getItem('savedFormulaPiecePackages');
    if (packages) {
      try {
        setSavedPackages(JSON.parse(packages));
      } catch (error) {
        console.error('Error loading saved formula piece packages:', error);
        setSavedPackages([]);
      }
    }
  }, []);
  ...
  const handleDeletePackage = (id: string) => {
    if (confirm('¿Estás seguro que deseas eliminar este paquete de fórmulas?')) {
      const updatedPackages = savedPackages.filter(pkg => pkg.id !== id);
      setSavedPackages(updatedPackages);
      localStorage.setItem('savedFormulaPiecePackages', JSON.stringify(updatedPackages));
  ...
  const handleLoadPackage = (packageData: SavedFormulaPiecePackage) => {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col sm:flex-row mx-2">
        ...
        <div className="w-full sm:w-1/2 border-b sm:border-b-0 sm:border-r border-gray-200 p-4 sm:p-6 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-[#003366]">Paquetes de Fórmulas Guardados</h2>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4">
            {savedPackages.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay paquetes de fórmulas guardados</p>
              </div>
            ) : (
