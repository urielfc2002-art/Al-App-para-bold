...
interface XXFormulaCustomizerProps {
  onClose: () => void;
  onSave: (formula: Formula) => void;
  line: 'L2' | 'L3';
}

const DEFAULT_FORMULAS_BY_LINE = {
  L3: {
...
export function XXFormulaCustomizer({ onClose, onSave, line }: XXFormulaCustomizerProps) {
  const [formula, setFormula] = useState<Formula>(() => {
    const formulaKey = `xxFormula_${line}`;
    const savedFormula = localStorage.getItem(formulaKey);
    return savedFormula ? JSON.parse(savedFormula) : DEFAULT_FORMULAS_BY_LINE[line];
  });
  ...
  const handleReset = () => {
    setFormula(DEFAULT_FORMULAS_BY_LINE[line]);
    setTempInputs({});
    const formulaKey = `xxFormula_${line}`;
    localStorage.removeItem(formulaKey);
    onSave(DEFAULT_FORMULAS_BY_LINE[line]);
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-[#003366]">
              Personalizar Fórmula - Línea Nacional de {line === 'L2' ? '2' : '3'}
            </h2>
            <button
              onClick={onClose}
