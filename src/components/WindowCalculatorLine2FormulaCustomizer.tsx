...
interface WindowCalculatorLine2FormulaCustomizerProps {
  onClose: () => void;
...
export function WindowCalculatorLine2FormulaCustomizer({ onClose, onSave }: WindowCalculatorLine2FormulaCustomizerProps) {
  const [formula, setFormula] = useState<Formula>(() => {
    const savedFormula = localStorage.getItem('windowCalculatorLine2Formula');
  ...
  const handleSave = () => {
    localStorage.setItem('windowCalculatorLine2Formula', JSON.stringify(formula));
    onSave(formula);
    setSaved(true);
  ...
  const handleReset = () => {
    ...
    setTempInputs({});
    localStorage.removeItem('windowCalculatorLine2Formula');
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Descuento Ventila Fija
                </label>
                <input
                  type="number"
                  value={getInputValue('ventilaFijaHeight')}
                  onChange={(e) => handleInputChange('ventilaFijaHeight', e.target.value)}
                  className="w-full px-3 py-2 border rounded-md text-sm"
                  step="0.1"
                />
              </div>
            ...
            {showPreview && (
              <div className="mt-4 p-4 bg-gray-50 rounded-md">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <p>Ventila Fija = Alto - {formula.ventilaFijaHeight} (1 PZ)</p>
                  <p>Ventila Corrediza = Alto - {formula.ventilaCorrHeight} (1 PZ)</p>
                  <p>Zoclo = (Ancho - {formula.zocloWidth}) / 2 (4 PZ)</p>
                </div>
              </div>
            )}
