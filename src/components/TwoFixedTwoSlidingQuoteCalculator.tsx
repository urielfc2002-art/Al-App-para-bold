...
import { TwoFixedTwoSlidingQuoteFormulaCustomizer } from './TwoFixedTwoSlidingQuoteFormulaCustomizer';
...
interface TwoFixedTwoSlidingQuoteCalculatorProps {
  onBack: () => void;
  onBackToNotes?: () => void;
  initialWidth?: string;
...
export function TwoFixedTwoSlidingQuoteCalculator({ onBack, onBackToNotes, initialWidth, initialHeight, showNotesButton = false }: TwoFixedTwoSlidingQuoteCalculatorProps) {
  console.log('🔍 TwoFixedTwoSlidingQuoteCalculator - Componente renderizado. Prop showNotesButton:', showNotesButton);

  const [width, setWidth] = useSyncedState<string>('two_fixed_two_sliding_quote_width', initialWidth || '');
  const [height, setHeight] = useSyncedState<string>('two_fixed_two_sliding_quote_height', initialHeight || '');
  ...
  const [glassAdjustment, setGlassAdjustment] = useSyncedState<number | string>('two_fixed_two_sliding_quote_glass_adjustment', 2);
  const [zocloSelection, setZocloSelection] = useSyncedState<ZocloSelection>('two_fixed_two_sliding_quote_zoclo', {
    upper: 'ZOCLO 1V_L3',
    lower: 'ZOCLO 1V_L3'
  });
  const [formula, setFormula] = useSyncedState<Formula>('two_fixed_two_sliding_quote_formula', {
  ...
  const [selectedHardware, setSelectedHardware] = useState<SelectedHardware[]>(() => {
    const savedHardware = localStorage.getItem('selectedTwoFixedTwoSlidingHardware');
    if (savedHardware) {
      try {
  ...
  const [selectedGlass, setSelectedGlass] = useState<SelectedGlass[]>(() => {
    const savedGlass = localStorage.getItem('selectedTwoFixedTwoSlidingGlass');
    if (savedGlass) {
      try {
  ...
  React.useEffect(() => {
    ...
    console.log('🔍 TwoFixedTwoSlidingQuoteCalculator - useEffect ejecutado con:', {
      initialWidth,
      initialHeight,
      currentWidth: width,
    ...
    if (initialWidth && initialWidth.trim() !== '') {
      console.log('🔍 TwoFixedTwoSlidingQuoteCalculator - Actualizando width a', initialWidth);
      setWidth(initialWidth);
    }
    if (initialHeight && initialHeight.trim() !== '') {
      console.log('🔍 TwoFixedTwoSlidingQuoteCalculator - Actualizando height a', initialHeight);
  ...
  useEffect(() => {
    localStorage.setItem('selectedTwoFixedTwoSlidingHardware', JSON.stringify(selectedHardware));
  }, [selectedHardware]);

  // Guardar vidrios seleccionados en localStorage cuando cambien
  useEffect(() => {
    localStorage.setItem('selectedTwoFixedTwoSlidingGlass', JSON.stringify(selectedGlass));
  }, [selectedGlass]);
  ...
  const calculateMeasurements = () => {
    ...
    const measurements = {
      ...
      cercochapa: {
        measure: (h - formula.ventilaCorrHeight).toFixed(1),
        pieces: 4
      },
      traslape: {
        measure: (h - formula.ventilaCorrHeight).toFixed(1),
  ...
  const calculateGlassMeasurements = () => {
    ...
    const glassWidth = parseFloat(results.measurements.zoclo.measure) + adjustmentValue;
  ...
  const calculateSquareMeters = (measurements: { width: string; heightFija: string; heightCorrediza: string } | null) => {
    if (!measurements) return null;

    const widthInMeters = parseFloat(measurements.width) / 100;
    const heightFijaInMeters = parseFloat(measurements.heightFija) / 100;
    const heightCorredizaInMeters = parseFloat(measurements.heightCorrediza) / 100;

    const areaFija = widthInMeters * heightFijaInMeters;
    const areaCorrediza = widthInMeters * heightCorredizaInMeters;
    const totalArea = (areaFija * 2) + (areaCorrediza * 2);
  ...
  return (
    <div className="min-h-screen bg-[#003366] flex flex-col items-center px-4 animate-fade-in">
      ...
      {showFormulaCustomizer && (
        <TwoFixedTwoSlidingQuoteFormulaCustomizer
