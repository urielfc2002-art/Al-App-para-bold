...
interface FormulaCustomizerProps {
  onClose: () => void;
...
export function FormulaCustomizer({ onClose, onSave }: FormulaCustomizerProps) {
  const [formula, setFormula] = useState<Formula>(() => {
    const savedFormula = localStorage.getItem('customFormula');
  ...
  const handleSave = () => {
    localStorage.setItem('customFormula', JSON.stringify(formula));
    onSave(formula);
    setSaved(true);
  ...
  const handleReset = () => {
    ...
    setTempInputs({});
    localStorage.removeItem('customFormula');
