...
interface SavedDoorFormulaPiecePackage {
  id: string;
  name: string;
  date: string;
  pieces: any[];
...
interface SavedDoorFormulaPiecePackagesModalProps {
  onClose: () => void;
  onLoadPackage: (packageData: SavedDoorFormulaPiecePackage) => void;
}

export function SavedDoorFormulaPiecePackagesModal({ onClose, onLoadPackage }: SavedDoorFormulaPiecePackagesModalProps) {
  const [savedPackages, setSavedPackages] = useState<SavedDoorFormulaPiecePackage[]>([]);
  const [selectedPackage, setSelectedPackage] = useState<SavedDoorFormulaPiecePackage | null>(null);

  useEffect(() => {
    const packages = localStorage.getItem('savedDoorFormulaPiecePackages');
    if (packages) {
      try {
        setSavedPackages(JSON.parse(packages));
      } catch (error) {
        console.error('Error loading saved door formula piece packages:', error);
        setSavedPackages([]);
      }
    }
  }, []);
  ...
  const handleDeletePackage = (id: string) => {
    if (confirm('¿Estás seguro que deseas eliminar este paquete de fórmulas de puertas?')) {
      const updatedPackages = savedPackages.filter(pkg => pkg.id !== id);
      setSavedPackages(updatedPackages);
      localStorage.setItem('savedDoorFormulaPiecePackages', JSON.stringify(updatedPackages));
  ...
  const handleLoadPackage = (packageData: SavedDoorFormulaPiecePackage) => {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col sm:flex-row mx-2">
        <div className="w-full sm:w-1/2 border-b sm:border-b-0 sm:border-r border-gray-200 p-4 sm:p-6 overflow-y-auto">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-[#003366]">Paquetes de Fórmulas de Puertas</h2>
            <button
              onClick={onClose}
          ...
          <div className="space-y-4">
            {savedPackages.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay paquetes de fórmulas de puertas guardados</p>
              </div>
            ) : (
                        ...
                        <div className="space-y-1">
                          {profilePieces.map((piece, index) => (
                            <div key={index} className="flex justify-between text-xs sm:text-sm">
                              <span className="text-gray-600">{piece.doorType}</span>
