...
interface FixedSlidingWindowComponentProps {
  fixedWidth: number;
  fixedHeight: number;
  windowWidth: string;
  windowHeight: string;
...
export function FixedSlidingWindowComponent({ 
  fixedWidth, 
  fixedHeight, 
  windowWidth, 
  windowHeight, 
  ...
  onExport
}: FixedSlidingWindowComponentProps) {
  // Estado para cada panel individual
  const [showArrowLeft, setShowArrowLeft] = useState(false);  // Panel FIJO (izquierdo)
  const [showArrowRight, setShowArrowRight] = useState(false); // Panel CORREDIZO (derecho)
  ...
  return (
    <div className="relative" style={{ width: fixedWidth, height: fixedHeight }}>
      ...
      <div className="w-full h-full bg-gradient-to-br from-amber-100 to-amber-200 border-2 border-[#003366] rounded">
        ...
        {/* Panel FIJO (izquierdo) - CON DOBLE CLICK */}
        <div 
        ...
        >
          {/* FLECHA APUNTANDO AL CENTRO (hacia la derecha →) */}
          {showArrowLeft && (
          ...
          {/* ÓVALO CAFÉ - EN LA ORILLA IZQUIERDA DEL PANEL IZQUIERDO */}
          {showArrowLeft && (
        ...
        {/* Panel CORREDIZO (derecho) - CON DOBLE CLICK */}
        <div 
        ...
        >
          {/* FLECHA APUNTANDO AL CENTRO (hacia la izquierda ←) */}
          {showArrowRight && (
            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="3">
          ...
          {/* ÓVALO CAFÉ - EN LA ORILLA DERECHA DEL PANEL DERECHO */}
          {showArrowRight && (
      ...
      <div className="absolute bottom-[-25px] sm:bottom-[-30px] left-1/2 transform -translate-x-1/2" style={{ zIndex: 10 }}>
        <AutoResizingInput
          value={windowWidth}
      ...
      {/* Campo de ALTO (izquierda) - BAJADO MÁS PARA MOSTRAR MANIJAS */}
