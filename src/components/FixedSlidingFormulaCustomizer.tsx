...
interface FixedSlidingFormulaCustomizerProps {
  onClose: () => void;
...
export function FixedSlidingFormulaCustomizer({ onClose, onSave }: FixedSlidingFormulaCustomizerProps) {
  const [formula, setFormula] = useState<Formula>(() => {
    const savedFormula = localStorage.getItem('fixedSlidingFormula');
  ...
  const handleSave = () => {
    localStorage.setItem('fixedSlidingFormula', JSON.stringify(formula));
    onSave(formula);
    setSaved(true);
  ...
  const handleReset = () => {
    ...
    setTempInputs({});
    localStorage.removeItem('fixedSlidingFormula');
            ...
            {showPreview && (
              <div className="mt-4 p-4 bg-gray-50 rounded-md">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <p>Ventila Fija = Alto - {formula.ventilaFijaHeight}</p>
                  <p>Ventila Corrediza = Alto - {formula.ventilaCorrHeight}</p>
                  <p>Zoclo = (Ancho - {formula.zocloWidth}) / 2</p>
                </div>
              </div>
            )}
