...
interface FixedSlidingCalculatorProps {
  onBack: () => void;
  onBackToNotes?: () => void;
  initialWidth?: string;
...
export function FixedSlidingCalculator({ onBack, onBackToNotes, initialWidth, initialHeight, showNotesButton = false }: FixedSlidingCalculatorProps) {
  console.log('🔍 FixedSlidingCalculator - Props recibidos:', {
    initialWidth,
    initialHeight,
    showNotesButton
  });
  console.log('🔍 FixedSlidingCalculator - Componente renderizado. Prop showNotesButton:', showNotesButton);

  const [width, setWidth] = useSyncedState<string>('fixedSlidingWidth', initialWidth || '');
  const [height, setHeight] = useSyncedState<string>('fixedSlidingHeight', initialHeight || '');
  ...
  const [glassAdjustment, setGlassAdjustment] = useSyncedState<number | ''>('fixed_sliding_line3_glass_adjustment', 2);
  ...
  const [zocloSelection, setZocloSelection] = useSyncedState<ZocloSelection>('fixedSlidingZoclo', {
    upper: 'ZOCLO 1V_L3',
    lower: 'ZOCLO 1V_L3'
  });
  const [formula, setFormula] = useSyncedState<Formula>('fixedSlidingFormula', {
  ...
  const [selectedHardware, setSelectedHardware] = useState<SelectedHardware[]>(() => {
    const savedHardware = localStorage.getItem('selectedAdditionalHardware');
    if (savedHardware) {
      try {
  ...
  const [selectedGlass, setSelectedGlass] = useState<SelectedGlass[]>(() => {
    const savedGlass = localStorage.getItem('selectedAdditionalGlass');
    if (savedGlass) {
      try {
  ...
  React.useEffect(() => {
    ...
    console.log('🔍 FixedSlidingCalculator - useEffect ejecutado con:', {
      initialWidth,
      initialHeight,
      currentWidth: width,
    ...
    if (initialWidth && initialWidth.trim() !== '') {
      console.log('🔍 FixedSlidingCalculator - Actualizando width a', initialWidth);
      setWidth(initialWidth);
    }
    if (initialHeight && initialHeight.trim() !== '') {
      console.log('🔍 FixedSlidingCalculator - Actualizando height a', initialHeight);
  ...
  useEffect(() => {
    localStorage.setItem('selectedAdditionalHardware', JSON.stringify(selectedHardware));
  }, [selectedHardware]);

  // Guardar vidrios seleccionados en localStorage cuando cambien
  useEffect(() => {
    localStorage.setItem('selectedAdditionalGlass', JSON.stringify(selectedGlass));
  }, [selectedGlass]);
  ...
  const calculateGlassMeasurements = () => {
    ...
    const glassFijaHeight = parseFloat(height) - formula.ventilaFijaHeight - upperZocloMeasure - lowerZocloMeasure + adjustment;
    ...
    return {
      ...
      heightFija: glassFijaHeight.toFixed(1),
  ...
  const calculateSquareMeters = () => {
    ...
    const heightFijaInMeters = parseFloat(glassMeasurements.heightFija) / 100;
    ...
    const totalArea = areaFija + areaCorrediza;

    return {
      areaFija: areaFija.toFixed(2),
        ...
        <div className="absolute z-20 w-64 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200">
          <div className="p-2">
            {['ZOCLO 1V_L3', 'ZOCLO 2V_L3', 'CABEZAL_L3'].map((profile) => {
              const displayName = profile.replace('_L3', '');
              return (
                <button
                  key={profile}
              ...
              <div className="space-y-4 text-[#003366]">
                ...
                <div className="border-b border-gray-200 pb-2">
                  ...
                  {zocloSelection.upper === zocloSelection.lower ? (
                    <div className="pl-4">
                      <div className="flex justify-between items-center">
                        <span className="text-sm">{zocloSelection.upper.replace('_L3', '')}:</span>
                        <div className="text-right">
                    ...
                    <div className="pl-4">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-sm">{zocloSelection.lower.replace('_L3', '')}:</span>
                        <div className="text-right">
                    ...
                    <div className="pl-4">
                      {selectedHardware.map((item, index) => (
                        <div key={index} className="flex justify-between items-center mt-1">
                          <span className="text-sm">{item.name.replace('_L3', '')}:</span>
                          <div className="text-right">
                    ...
                    <div className="pl-4">
                      {selectedGlass.map((item, index) => (
                        <div key={index} className="flex justify-between items-center mt-1">
                          ...
                          <div className="text-right">
                            <div className="font-bold">{item.quantity} {item.chargingMethod === 'piece' ? 'PIEZA' : 'M²'}{item.quantity > 1 ? 'S' : ''}</div>
                            <div className="text-sm text-gray-600">${item.total.toFixed(2)}</div>
                          </div>
                        </div>
                      ))}
        ...
        >
          ...
          >
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <h2 className="text-2xl font-bold text-[#003366]">Contemplar Vidrio - Fijo Corredizo L3</h2>
              <button
                onClick={() => setShowGlassModal(false)}
              ...
              {glassMeasurements && (
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-r from-orange-50 to-orange-100 rounded-lg p-6 border border-orange-200">
                    <h4 className="text-lg font-bold text-orange-800 mb-3">VENTILA FIJA</h4>
                  ...
                  {squareMeters && (
                    <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-6 border border-purple-200">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-purple-700 font-medium">Ventila Fija:</span>
                          <span className="text-purple-900 font-bold text-lg">{squareMeters.areaFija} m²</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-purple-700 font-medium">Ventila Corrediza:</span>
